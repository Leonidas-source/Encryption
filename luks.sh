...
use_conf_file() {
  clear
  answr3=$(cat luks.conf)
}
fs1() {
  ls | grep -w "luks.conf" && use_conf_file || (echo "set name for your container" && read answr3 && echo $answr3 | cat >> luks.conf)
  answr3=$(cat luks.conf)
  ...
  [ "$fs_for_container" == "2" ] && mkfs.btrfs /dev/mapper/$answr3
